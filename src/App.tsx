...
const BackToDashboardBtn = ({ onClick, text }: { onClick: () => void, text: string }) => (
  <div className="mb-8 w-full max-w-[1600px] mx-auto px-4 sm:px-8 relative z-20 text-left">
    <button type="button" onClick={onClick} className="flex items-center text-[#EA580C] font-black bg-[#0F172A] px-6 py-3 rounded-xl shadow-xl w-fit group hover:bg-black transition-all border border-slate-800">
...
export default function App() {
  ...
  const executeLogin = () => { setIsLoggedIn(true); setAuthModal({ open: false, mode: null, step: 'role' }); handlePageChange('dashboard'); };

  const handleLogout = () => { setIsLoggedIn(false); setUser({ firstName: '', lastName: '', mobile: '', email: '', password: '', confirmPassword: '', otp: '', businessName: '', gst: '', dl: '', dp: '' }); setSelectedRole(null); handlePageChange('landing'); };
        ...
        <div className="max-w-[1600px] mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            {isLoggedIn ? (
              ...
              <>
                <button type="button" onClick={() => setAuthModal({ open: true, mode: 'login', step: 'role' })} className="text-[#0F172A] font-black text-sm hover:text-[#EA580C] transition-colors px-4">
                  Login
                </button>
                <button type="button" onClick={() => setAuthModal({ open: true, mode: 'register', step: 'role' })} className="bg-[#0F172A] text-white px-7 py-3 rounded-xl font-black text-sm hover:bg-slate-800 transition-colors shadow-lg">
                  Create Account
                </button>
          ...
          <div className="relative w-80 bg-white h-full shadow-2xl flex flex-col animate-fade-in-right z-50 border-l border-slate-100">
            {isLoggedIn ? (
              ...
              <div className="flex flex-col items-center justify-center h-full p-10 text-center space-y-6">
                ...
                <button type="button" onClick={() => { setDrawerOpen(false); setAuthModal({ open: true, mode: 'login', step: 'role' }); }} className="bg-[#EA580C] text-white px-10 py-4 rounded-xl font-black text-lg w-full mt-4 shadow-xl hover:bg-orange-700 transition-colors">Login Now</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-[#0F172A]/90 backdrop-blur-md animate-fade-in pointer-events-auto">
          <div className="w-full max-w-4xl bg-white rounded-[2rem] shadow-2xl flex overflow-hidden h-[550px] border border-slate-200 relative">
            ...
            <div className="w-full md:w-1/2 p-8 sm:p-12 relative flex flex-col justify-center bg-white pointer-events-auto">
              <button type="button" onClick={() => setAuthModal({ open: false, mode: null, step: 'role' })} className="absolute top-6 right-6 text-slate-400 hover:text-slate-900 bg-slate-50 p-2 rounded-xl border z-50"><X className="h-5 w-5" /></button>
                ...
                <div className="space-y-8 animate-fade-in text-center relative z-20">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      }} className="p-5 rounded-xl border border-slate-200 bg-white hover:border-[#EA580C] hover:shadow-md transition-all font-black text-slate-700 text-sm shadow-sm hover:text-[#EA580C]">
                        {r.label}
                      </button>
                    ))}
                  </div>
              ...
              {authModal.step === 'details' && (
                <div className="space-y-5 animate-fade-in overflow-y-auto max-h-full pb-4 pr-2 relative z-20">
                  ...
                  }} className="w-full bg-[#0F172A] text-white font-black py-4 rounded-xl shadow-lg mt-6 hover:bg-slate-800 transition-colors text-base">Create Account</button>
                </div>
              )}
              ...
              {authModal.step === 'mobile' && (
                <div className="space-y-8 animate-fade-in text-center relative z-20">
                  ...
                  <div className="flex flex-col gap-4 pt-2">
                    <button type="button" onClick={() => setAuthModal({ ...authModal, step: 'password' })} disabled={user.mobile.length !== 10} className="w-full bg-[#0F172A] disabled:bg-slate-300 text-white font-black py-4 rounded-xl transition-colors shadow-lg hover:bg-slate-800 text-lg">Continue</button>
                  </div>
                </div>
              )}
              ...
              {authModal.step === 'otp' && (
                <div className="space-y-6 animate-fade-in text-center relative z-20">
                  <h3 className="text-3xl font-black text-slate-900 tracking-tight mb-2">Verify Number</h3>
                  ...
                  <button type="button" onClick={async () => {
                    ...
                    try {
                      const response = await fetch("https://y2wait-backend.onrender.com/api/auth/verify-otp", {
                        ...
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                          mobileNum: user.mobile, // Changed from mobile to mobileNum
                          otp: otpVal
                        })
                      });
                      if (response.ok) {
                  ...
                  }} disabled={otpVal.length !== 4} className="w-full bg-[#EA580C] disabled:bg-slate-300 text-white font-black py-4 rounded-xl shadow-lg mt-6 transition-colors text-base">Verify & Login</button>
                </div>
              )}
                      ...
                      <div key={header.id} className={`space-y-5 transition-all duration-1000 absolute bottom-10 left-10 right-10 ${i === heroIdx ? 'opacity-100 translate-y-0 z-20 pointer-events-auto' : 'opacity-0 translate-y-10 z-0 pointer-events-none'}`}>
                        ...
                        <div className="flex space-x-4 mt-6">
                          ...
                          {(header.btn2 === 'Join Network') && (
                            <button type="button" onClick={() => setAuthModal({ open: true, mode: 'register', step: 'role' })} className="bg-[#EA580C] text-white px-8 py-4 rounded-xl font-black text-sm hover:bg-orange-700 transition-colors flex items-center shadow-xl hover:scale-105 transform cursor-pointer">
          ...
          <section className="max-w-[1200px] mx-auto px-4 sm:px-8 py-20 animate-fade-in z-10 relative">
            ...
            <div className="bg-white rounded-[3rem] overflow-hidden shadow-2xl border border-slate-200">
              ...
              <div className="p-16 space-y-10">
                ...
                <button type="button" onClick={() => setAuthModal({ open: true, mode: 'register', step: 'role' })} className="bg-[#0F172A] text-white px-12 py-5 rounded-2xl text-xl font-black hover:bg-slate-800 transition-colors shadow-2xl">Start Using This Feature</button>
              </div>
            </div>
          </section>
        )}
            ...
            {(activeModule === 'freight' || activeModule === 'bus_cargo') && (
              <div className="flex justify-center mb-10">
                <div className="flex bg-slate-100 p-2 rounded-2xl w-fit border border-slate-200 shadow-inner">
                  <button type="button" onClick={() => { if (activeModule === 'freight') { setListingTab('all'); } else { setBusTab('all'); } }} className={`px-10 py-3.5 rounded-xl text-sm font-black transition-all ${(activeModule === 'freight' ? listingTab : busTab) === 'all' ? 'bg-white text-[#EA580C] shadow' : 'text-slate-500 hover:text-slate-900'}`}>Market Feed</button>
                  <button type="button" onClick={() => { if (activeModule === 'freight') { setListingTab('my_listings'); } else { setBusTab('my_listings'); } }} className={`px-10 py-3.5 rounded-xl text-sm font-black transition-all ${(activeModule === 'freight' ? listingTab : busTab) === 'my_listings' ? 'bg-white text-[#EA580C] shadow' : 'text-slate-500 hover:text-slate-900'}`}>My Listings</button>
                  <button type="button" onClick={() => { if (activeModule === 'freight') { setListingTab('command_center'); } else { setBusTab('command_center'); } }} className={`px-10 py-3.5 rounded-xl text-sm font-black transition-all ${(activeModule === 'freight' ? listingTab : busTab) === 'command_center' ? 'bg-[#0F172A] text-white shadow' : 'text-slate-500 hover:text-slate-900'}`}>Radar</button>
                </div>
              </div>
            )}
